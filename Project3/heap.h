...
class heap {
    ...
    int percolateUp(int key); // Percolates up a given key
    int percolateDown(int key); // Percolates down a given key
    int getPosition(node *pn); // Finds the position of a node in data, returns the position or -1 if error
    int decreaseKey(int pos, int key);
    int increaseKey(int pos, int key);
};
